...
- **Post-pass (`SpringCompileFixups`):** Spring 6 `HttpStatusCode` phrase handling, Neo4j `ServerAddressResolver` `List`→`Set`, servlet header access (`getHeaders().get`→`Optional.ofNullable(getHeader)`), `getErrorResult(UNAUTHORIZED)`→`HttpStatus.UNAUTHORIZED.value()`, controller `CompletableFuture.supplyAsync` generics, `ResponseEntity.ok("…")` for `JsonNode` returns.
